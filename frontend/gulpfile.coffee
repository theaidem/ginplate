...
gulp.task 'cjsx', ['clean'], ->
	gulp.src "#{src.coffee}**/*.coffee"
		...
		.pipe( cjsx( {bare: true, sourcemap: {inline: true}} ) )
			.on( 'error', notify.onError('Error: <%= error.message %>') )
			.on( 'error', gutil.log )
			.on( 'error', gutil.beep )
		.pipe( sourcemaps.write() )
		.pipe( gulp.dest("#{app}/js/" ) )    
		.pipe( notify({ onLast: true, message:'Coffee (cjsx) compile with success!' }) )
...
gulp.task 'compile', (callback) ->
	runSequence [ 'bowerInstall', 'stylus', 'cjsx', 'jade' ], callback
